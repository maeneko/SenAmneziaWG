...
- Автозапуск при входе — через `app.setLoginItemSettings`, Electron сам пишет `.desktop` в
  `~/.config/autostart`.
- Обновление — тот же `.run`, скачанный и перезапущенный (`update/server.ts`: `os=linux`, только x64,
...
- **Агент polkit не обязателен.** Пароль администратора (установка, обновление) спрашивает агент
  рабочего стола. На голом оконном менеджере (Hyprland, sway, i3) его часто нет, и pkexec без терминала
  падает с «Error creating textual authentication agent». Только в этом случае установщик запускает
  pkexec повторно на своём псевдотерминале (`script` из util-linux) и спрашивает пароль на своём экране;
  при обновлении окно появляется поверх приложения с полем пароля (`src/main/setup/elevateLinux.ts`).
  Удаление из настроек так не умеет — без агента оно скажет, что агента нет.
...
1. `npm run build:linux` действительно собирает `dist/SenAWG-<version>-linux-x64.run`,
   и он реально запускается на чистой системе (архитектура электрон-билдера для `${arch}` в
...
9. Работа в фоне и трей: показывается ли значок в GNOME без AppIndicator, в KDE, в Ubuntu (значок с
   собственным AppIndicator из коробки).
10. Бесшовное обновление (описано в `docs/installer.md`): пока идёт подготовка, приложение работает; окно
