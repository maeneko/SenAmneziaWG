...
// AmnesiaWG. Choosing D:\Programs puts it in D:\Programs\AmnesiaWG, not loose among everything else in
...
func AppDir(picked string) string {
	...
	if strings.EqualFold(p[cut+1:], "AmnesiaWG") {
		return p
	}
	// «D:» alone is a drive root; the separator goes back on so the result is D:\AmnesiaWG.
	return p + `\AmnesiaWG`
}
